...
import { addDinerMissionToDB, getDinerMissionById, checkDinerExists, checkMissionExists, getAllDinerMissions, } from "./mission.repository.js";
export const addDinerMission = async (dinerId, data) => {
    // 1. 가게 존재 여부 검증
...
export const listDinerMission = async (dinerId, cursor) => {
    // 1. 가게 존재 여부 검증
    const dinerExists = await checkDinerExists(dinerId);
    if (!dinerExists) {
        throw new DinerNotFoundError(`ID가 ${dinerId}인 가게를 찾을 수 없습니다.`);
