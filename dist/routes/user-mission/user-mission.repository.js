...
export const checkOngoingMission = async (userId, dinerMissionId) => {
    try {
...
// 미션 도전 로그 추가 (진행중 상태로)
export const addMissionClearLog = async (userId, dinerMissionId) => {
    try {
...
// 미션 도전 로그 단건 조회
export const getMissionClearLogById = async (logId) => {
    try {
...
export const getMyMissionListRepo = async (userId, cursor) => {
    try {
...
//// ===== 6주차 미션 4 ======
// 1. 해당 미션의 유저가 맞는지 체크
export const checkUser = async (userId, missionLogId) => {
...
// 2. 현재 진행중인 미션인지 체크
export const checkMissionProgress = async (missionLogId) => {
    const checkMissionProgress = await prisma.missionClearLog.findUnique({
        where: {
            id: missionLogId,
            status: "진행중"
        }, // 미션 로그인지 
...
export const patchCompleteMission = async (userId, missionLogId) => {
    try {
