...
import { UpdateUserData, UserSignUpRequest } from "./user.types.js";
...
// ==== 회원 정보 수정 API ====
export const handleUserPatch = async (req: Request, res: Response, next: NextFunction) => {
  /*
...
*/
  ...
  const userId = (req.user as any).id; // JWT 토큰에서 추출한 User ID
  console.log("body:", req.body); // 값이 잘 들어오나 확인하기 위한 테스트용

  const user = await userPatch(userId, bodyToUpdatedUser(req.body as UpdateUserData)); // DTO 변환 및 Service 로직으로 전달
